...
\usepackage{amsmath, graphicx, blindtext, tikz}
...
% affiliations
\author{\IEEEauthorblockN{Mandar Chandorkar\IEEEauthorrefmark{1}, Oliver Lauwers, Raghvendra Mall, Johann A.K Suykens, Bart De Moor}
...
In the following section we introduce the FS-LSSVM algorithm\cite{DeBrabanter2010}. Section \ref{archi} outlines the various modules that comprise Bayes Learn and their roles. An implementation of the FS-LSSVM model is constructed using the framework and tested on various data sets, with the findings outlined in \ref{exp}. Finally, we conclude in Section \ref{Conclusion}.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Least Squares Support Vector Machines}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Formulation}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
Least Squares Support Vector Machines (LSSVM) \cite{Suykens1999} introduces in the SVM formulation the \textit{squared error} loss function and equality constraints with respect to the slack variables $e_i$, as shown in equation \ref{eqlssvm}. 
\begin{align}
\label{eqlssvm}
& min_{w,b,e} \ \frac{1}{2}w^{T} w + \frac{\gamma}{2}\sum^{N}_{k=1}e_k^{2} \\   
& s.t \ Y_{k}[w^{T} \phi(X_k) + b] = 1 - e_{k}, k=1,\ldots,N
\end{align}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\widehat{b}
\end{matrix}\right ) = 
\left ( \widehat{\Phi}^{T}_e \widehat{\Phi}_e + \frac{\mathit{I}_{m+1}}{\gamma} \right )^{-1} \widehat{\Phi}^{T}_e Y
\\ \\
& \widehat{\Phi}_e = \begin{pmatrix}
\widehat{\phi}_{1}(X_1) & \cdots & \widehat{\phi}_{m}(X_1) & 1\\ 
\vdots &  \ddots & \vdots & \vdots\\ 
\widehat{\phi}_{1}(X_n) & \cdots & \widehat{\phi}_{m}(X_n) & 1
\end{pmatrix}
\end{align*}

In the above formulation, $\widehat{\phi}(X_k)$ is an approximation to the true feature map $\phi(X_k)$ which is related to the kernel $K(X_i, X_j) = \phi(X_i)^T \phi(X_j)$ due to Mercer's theorem. The approximate feature map $\widehat{\phi}(X_k)$ is calculated using the Nystrom method as outlined in Brabanter \cite{DeBrabanter2010}. A low rank approximation to the kernel matrix is constructed by iteratively calculating a subset of the original data which maximises the \textit{Quadratic Renyi Entropy}. This procedure of extracting $\widehat{\phi}(X_k)$ from a data set, given a kernel function, is called \textit{Automatic Feature Extraction} (AFE).

Kernel based models suffer from sensitivity to hyper-parameters, in the case of FS-LSSVM we have to tune the model with respect to $\gamma$ the regularization parameter and the parameters of the kernel chosen. Models are generally compared with their cross validation performance in which case the objective cost function with respect to the hyper-parameters is in general non smooth. Gradient free methods like Grid Search, Nelder Mead \cite{Nelder1965} and Coupled Simulated Annealing \cite{Xavier-De-Souza2010} are suitable to tackle the model selection problem for FS-LSSVM based models.
...
Figure \ref{fig:struct} shows the organization of modules in Bayes Learn. It can be decomposed into five principal modules.
\begin{itemize}
\item Model Classes:
...
A module which houses the implementation of common optimization methods (i.e. Gradient and Gradient free). Currently Bayes Learn has implementations for Conjugate Gradient, Gradient Descent, Grid Search and Coupled Simulated Annealing \cite{Xavier-De-Souza2010}. 
\item Kernels:
Bayes Learn is equipped with a powerful abstract API for representing kernel functions. The module has two abstract classes to outline the behaviors of kernels used in SVM based applications as well as density estimation. The library comes bundled with an implementation for AFE as well as for common SVM kernels i.e. RBF, Polynomial, Laplace, Exponential. New kernel functions can be easily added to the library by extending the base classes in this module.
...
\end{tikzpicture}
\end{adjustbox}
\caption{Schematic structure of Bayes Learn}
\label{fig:struct}
\end{figure}
